<div class="prices_top_info mb50">
    <p>!!! с 01.03.2017 года в клинике Доступная Стоматология действуют следующие скидки:<br>
        - семейная скидка 10% - просто назовите имя и фамилию члена вашей семьи, который уже у нас лечился (лечится);<br>
        - скидка в размере  50 % на все услуги, указанные  в разделе "Гигиена".<br>
        - скидка в размере  10 % на все услуги, указанные  в разделе "Имплантология".</p>
</div>

<div class="panel-group mb50">
    <div class="panel panel-default">
        <a class="panel-heading btn" data-toggle="collapse" href="#price_therapy" >Терапия</a>
        <div id="price_therapy" class="panel-collapse collapse">
            <table width="100%" border="0" cellspacing="2" cellpadding="0">
                <tr>
                    <th>Услуга</th>
                    <th width="100" align="center">Цена руб.</th>
                </tr>
                <tr>
                    <td>Осмотр, консультация, заполнение мед. документации</td>
                    <td width="100" align="center">250</td>
                </tr>

                <tr>
                    <td colspan="2" align="center"><strong>ЛЕЧЕНИЕ (КАРИЕС, ПУЛЬПИТ, ПЕРИОДОНТИТ)</strong></td>
                </tr>

                <tr>
                    <td colspan="2" align="center"><strong>Световые композиты</strong></td>
                </tr>
                <tr>
                    <td>Charisma, Te-Econom</td>
                    <td align="center">1500</td>
                </tr>
                <tr>
                    <td>Filtek Ultimate, Estelite, Premise</td>
                    <td align="center">2000</td>
                </tr>

                <tr>
                    <td colspan="2" align="center"><strong>Изолирующая подкладка</strong></td>
                </tr>

                <tr>
                    <td>Vitrebond</td>
                    <td align="center">400</td>
                </tr>

                <tr>
                    <td>Estelite Flow Quik</td>
                    <td align="center">500</td>
                </tr>

                <tr>
                    <td>Natural Flow</td>
                    <td align="center">500</td>
                </tr>

                <tr>
                    <td>Calcimol LC</td>
                    <td align="center">500</td>
                </tr>

                <tr>
                    <td>Ionosit</td>
                    <td align="center">500</td>
                </tr>

                <tr>
                    <td>SDR</td>
                    <td align="center">500</td>
                </tr>

                <tr>
                    <td colspan="2" align="center"><strong>Лечение ПУЛЬПИТА, ПЕРИОДОНТИТА, пломбирование канала временное</strong></td>
                </tr>

                <tr>
                    <td>1канал</td>
                    <td align="center">750</td>
                </tr>

                <tr>
                    <td>2канала</td>
                    <td align="center">1250</td>
                </tr>

                <tr>
                    <td>3канала</td>
                    <td align="center">1750</td>
                </tr>

                <tr>
                    <td>4канала</td>
                    <td align="center">2000</td>
                </tr>

                <tr>
                    <td colspan="2" align="center"><strong>Лечение ПУЛЬПИТА, ПЕРИОДОНТИТА, пломбирование канала постоянное</strong></td>
                </tr>

                <tr>
                    <td>1канал</td>
                    <td align="center">1500</td>
                </tr>

                <tr>
                    <td>2канала</td>
                    <td align="center">2200</td>
                </tr>

                <tr>
                    <td>3канала</td>
                    <td align="center">3000</td>
                </tr>

                <tr>
                    <td>4канала</td>
                    <td align="center">3500</td>
                </tr>

                <tr>
                    <td colspan="2" align="center"><strong>Штифты</strong></td>
                </tr>
                <tr>
                    <td>Unimetric титановый</td>
                    <td align="center">600</td>
                </tr>
                <tr>
                    <td>Стекловолоконный</td>
                    <td align="center">1700</td>
                </tr>

                <tr>
                    <td colspan="2" align="center"><strong>ДОПОЛНИТЕЛЬНЫЕ РАБОТЫ</strong></td>
                </tr>
                <tr>
                    <td>Наложение девитализирующей пасты</td>
                    <td align="center">150</td>
                </tr>
                <tr>
                    <td>Наложение лекарства под временную пломбу</td>
                    <td align="center">400</td>
                </tr>
                <tr>
                    <td>Коагуляция десневого сосочка</td>
                    <td align="center">100</td>
                </tr>
                <tr>
                    <td>Ретракция десневого края</td>
                    <td align="center">100</td>
                </tr>
                <tr>
                    <td>Изготовление силиконового ключа</td>
                    <td align="center">300</td>
                </tr>
                <tr>
                    <td>Изоляция поля Оплдамм</td>
                    <td align="center">300</td>
                </tr>
                <tr>
                    <td>Временная фиксация коронки 1 ед.</td>
                    <td align="center">250</td>
                </tr>
                <tr>
                    <td>Снятие коронки из металла 1 ед.</td>
                    <td align="center">250</td>
                </tr>
                <tr>
                    <td>Снятие коронки из металлокерамики 1 ед.</td>
                    <td align="center">500</td>
                </tr>
                <tr>
                    <td>Шлифовка пломбы, сколов</td>
                    <td align="center">200</td>
                </tr>
                <tr>
                    <td>Закрытие перфорации</td>
                    <td align="center">500</td>
                </tr>
                <tr>
                    <td>Временная пломба</td>
                    <td align="center">100</td>
                </tr>
                <tr>
                    <td>Удаление инородного тела из канала</td>
                    <td align="center">700</td>
                </tr>
                <tr>
                    <td>Распломбировка каналов, 1 канал</td>
                    <td align="center">300</td>
                </tr>

            </table>
        </div>
    </div>
    <div class="panel panel-default">
        <a class="panel-heading btn" data-toggle="collapse" href="#price_childrens" >Детский прием</a>
        <div id="price_childrens" class="panel-collapse collapse">
            <table width="100%" border="0" cellspacing="2" cellpadding="0">
                <tr>
                    <th>Услуга</th>
                    <th width="100" align="center">Цена руб.</th>
                </tr>
                <tr>
                    <td>Осмотр, консультация, заполнение мед. документации</td>
                    <td width="100" align="center">250</td>
                </tr>

                <tr>
                    <td>Герметизация фиссур  1 зуб</td>
                    <td align="center">400</td>
                </tr>
                <tr>
                    <td>Фторирование зубов 1 зуб</td>
                    <td align="center">150</td>
                </tr>
                <tr>
                    <td colspan="2" align="center"><strong>МОЛОЧНЫЕ ЗУБЫ</strong></td>
                </tr>
                <tr>
                    <td>Серебрение 1 зуба</td>
                    <td align="center">150</td>
                </tr>
                <tr>
                    <td>Лечение КАРИЕСА СРЕДНЕГО (включая осмотр, анестезию)</td>
                    <td align="center">1000</td>
                </tr>
                <tr>
                    <td>Лечение КАРИЕСА ГЛУБОКОГО (включая осмотр, анестезию)</td>
                    <td align="center">1100</td>
                </tr>
                <tr>
                    <td>Лечение ПУЛЬПИТА в одно посещение</td>
                    <td align="center">1600</td>
                </tr>
                <tr>
                    <td>Лечение ПУЛЬПИТА 1-е посещение (включая осмотр)</td>
                    <td align="center">700</td>
                </tr>
                <tr>
                    <td>Лечение ПУЛЬПИТА 1-е, 2-е посещение (включая осмотр)</td>
                    <td align="center">400</td>
                </tr>
                <tr>
                    <td>Лечение ПЕРИОДОНТИТА 1-е посещение (включая осмотр)</td>
                    <td align="center">700</td>
                </tr>
                <tr>
                    <td>Лечение ПУЛЬПИТА и ПЕРИОДОНТИТА 2-е посещение (включая осмотр) с постановкой пломбы</td>
                    <td align="center">1500</td>
                </tr>
                <tr>
                    <td>УДАЛЕНИЕ молочного зуба (включая осмотр, анестезию)</td>
                    <td align="center">450</td>
                </tr>
                <tr>
                    <td>УДАЛЕНИЕ подвижного молочного зуба (включая осмотр, анестезию)</td>
                    <td align="center">200</td>
                </tr>
                <tr>
                    <td>иссечение капюшона ретинированного зуба  (включая осмотр, анестезию)</td>
                    <td align="center">500</td>
                </tr>
                <tr>
                    <td colspan="2" align="center"><strong>ДОПОЛНИТЕЛЬНЫЕ РАБОТЫ</strong></td>
                </tr>
                <tr>
                    <td>Наложение девитализирующей пасты</td>
                    <td align="center">150</td>
                </tr>
                <tr>
                    <td>Коагуляция десневого сосочка</td>
                    <td align="center">100</td>
                </tr>
                <tr>
                    <td>Ретракция десневого края</td>
                    <td align="center">100</td>
                </tr>
                <tr>
                    <td>Раскрытие полости зуба (ампутация, экстирпация пульпы)</td>
                    <td align="center">150</td>
                </tr>
                <tr>
                    <td>Изготовление силиконового ключа</td>
                    <td align="center">300</td>
                </tr>
                <tr>
                    <td>Изоляция поля Оплдамм</td>
                    <td align="center">300</td>
                </tr>
                <tr>
                    <td>Шлифовка пломбы, сколов</td>
                    <td align="center">200</td>
                </tr>
                <tr>
                    <td>Лечебная повязка при пульпите/периодонтите</td>
                    <td align="center">150</td>
                </tr>
            </table>
        </div>
    </div>
    <div class="panel panel-default">
        <a class="panel-heading btn" data-toggle="collapse" href="#price_hygiene" >Гигиена</a>
        <div id="price_hygiene" class="panel-collapse collapse">
            <table width="100%" border="0" cellspacing="2" cellpadding="0">
                <tr>
                    <th>Услуга</th>
                    <th width="100" align="center">Цена руб.</th>
                </tr>
                <tr>
                    <td>Осмотр, консультация, заполнение мед. документации</td>
                    <td width="100" align="center">250</td>
                </tr>
                <tr>
                    <td>Повторный осмотр</td>
                    <td align="center">100</td>
                </tr>
                <tr>
                    <td>Наложение ретрактора (защита десны)</td>
                    <td align="center">300</td>
                </tr>

                <tr>
                    <td colspan="2" align="center"><strong>ПРОФЕССИОНАЛЬНАЯ ГИГИЕНА ПОЛОСТИ РТА</strong></td>
                </tr>
                <tr>
                    <td>Индикация налета на поверхности эмали зубов (1 челюсть)</td>
                    <td align="center">100</td>
                </tr>
                <tr>
                    <td>Чистка зубного налета  (ультразвук, аэрфло, шлифовка) 1 зуб</td>
                    <td align="center">100</td>
                </tr>
                <tr>
                    <td>Чистка зубного налета  (ультразвук, аэрфло, шлифовка) 6 передних зубов </td>
                    <td align="center">1000</td>
                </tr>
                <tr>
                    <td>Чистка зубного налета  (ультразвук, аэрфло, шлифовка) 1 челюсть</td>
                    <td align="center">1500</td>
                </tr>
                <tr>
                    <td>Финишная шлифовка и полировка зубов пастой 1 зуб</td>
                    <td align="center">60</td>
                </tr>
                <tr>
                    <td>Чистка  налета курильщика, камней  (ультразвук, аэрфло, шлифовка) 1 зуб</td>
                    <td align="center">350</td>
                </tr>
                <tr>
                    <td>Чистка налета курильщика, камней  (ультразвук, аэрфло, шлифовка) 6 передних зубов </td>
                    <td align="center">1500</td>
                </tr>
                <tr>
                    <td>Чистка  налета курильщика, камней (ультразвук, аэрфло, шлифовка) 1 челюсть</td>
                    <td align="center">2000</td>
                </tr>
                <tr>
                    <td>Отбеливание Opalescence по показаниям 1 сеанс, 1 челюсть</td>
                    <td align="center">6000</td>
                </tr>

            </table>
        </div>
    </div>
    <div class="panel panel-default">
        <a class="panel-heading btn" data-toggle="collapse" href="#price_surgery" >Хирургия</a>
        <div id="price_surgery" class="panel-collapse collapse">
            <table width="100%" border="0" cellspacing="2" cellpadding="0">
                <tr>
                    <th>Услуга</th>
                    <th width="100" align="center">Цена руб.</th>
                </tr>
                <tr>
                    <td>Осмотр, консультация, заполнение мед. документации</td>
                    <td width="100" align="center">250</td>
                </tr>

                <tr>
                    <td>Удаление зуба простое (включая осмотр)</td>
                    <td align="center">1000</td>
                </tr>
                <tr>
                    <td>Удаление зуба сложное (включая осмотр)</td>
                    <td align="center">1500</td>
                </tr>
                <tr>
                    <td>Удаление ретенированного зуба (включая осмотр)</td>
                    <td align="center">2500</td>
                </tr>
                <tr>
                    <td>Разрез</td>
                    <td align="center">200</td>
                </tr>
                <tr>
                    <td>Иссечение капюшона ретенированного зуба (включая осмотр)</td>
                    <td align="center">1000</td>
                </tr>
                <tr>
                    <td>Лечение альвеолита (включая осмотр, анестезию)</td>
                    <td align="center">450</td>
                </tr>
                <tr>
                    <td>Наложение швов на лунку (отечественный материал)</td>
                    <td align="center">200</td>
                </tr>
                <tr>
                    <td>Наложение швов на лунку (импортный материал)</td>
                    <td align="center">450</td>
                </tr>
                <tr>
                    <td>Снятие шва</td>
                    <td align="center">100</td>
                </tr>
                <tr>
                    <td>Повторный осмотр — динамическое наблюдение</td>
                    <td align="center">150</td>
                </tr>
                <tr>
                    <td>Перевязка</td>
                    <td align="center">150</td>
                </tr>
                <tr>
                    <td>Коррекция объёма и формы альвиолярного отростка (без обезбаливания) в пределах  от 1 до 3 зубов, включая наложение швов</td>
                    <td align="center">1000</td>
                </tr>
                <tr>
                    <td>Резекция верхушки корня (одного зуба)</td>
                    <td align="center">2000</td>
                </tr>
                <tr>
                    <td>Цистотомия или цистэктомия, гранулэтомия</td>
                    <td align="center">1400</td>
                </tr>
                <tr>
                    <td>Пластика перфорации верхнечелюстной пазухи</td>
                    <td align="center">1000</td>
                </tr>

            </table>
        </div>
    </div>
    <div class="panel panel-default">
        <a class="panel-heading btn" data-toggle="collapse" href="#price_orthopedics" >Ортопедия</a>
        <div id="price_orthopedics" class="panel-collapse collapse">
            <table width="100%" border="0" cellspacing="2" cellpadding="0">
                <tr>
                    <th>Услуга</th>
                    <th width="100" align="center">Цена руб.</th>
                </tr>
                <tr>
                    <td>Осмотр, консультация, заполнение мед. документации</td>
                    <td width="100" align="center">250</td>
                </tr>

                <tr>
                    <td>Коронка металокерамика одна</td>
                    <td align="center">7150</td>
                </tr>
                <tr>
                    <td>Коронка металокерамика вторая и следующая каждая</td>
                    <td align="center">6100</td>
                </tr>
                <tr>
                    <td>Коронка литая</td>
                    <td align="center">5000</td>
                </tr>
                <tr>
                    <td>Коронка литая вторая и следующая каждая</td>
                    <td align="center">4000</td>
                </tr>
                <tr>
                    <td>Коронка пластмассовая (лабораторная)</td>
                    <td align="center">2000</td>
                </tr>
                <tr>
                    <td>Коронка, вкладка, винир е-мах</td>
                    <td align="center">13200</td>
                </tr>
                <tr>
                    <td>Коронка на импланте</td>
                    <td align="center">13200</td>
                </tr>
                <tr>
                    <td>Вкладка штиштовая</td>
                    <td align="center">5500</td>
                </tr>
                <tr>
                    <td>Съемный протез</td>
                    <td align="center">13200</td>
                </tr>
                <tr>
                    <td>Съемный протез из термомассы</td>
                    <td align="center">22000</td>
                </tr>
                <tr>
                    <td>Бюгельный протез из термосмолы</td>
                    <td align="center">27000</td>
                </tr>
                <tr>
                    <td>Бюгельный протез литой</td>
                    <td align="center">22000</td>
                </tr>
                <tr>
                    <td>Временные коронки 1-4</td>
                    <td align="center">2750</td>
                </tr>
                <tr>
                    <td>Временные коронки 5-10</td>
                    <td align="center">3850</td>
                </tr>
                <tr>
                    <td>Косметическая пластинка до трех зубов</td>
                    <td align="center">5600</td>
                </tr>
                <tr>
                    <td>Приварка зуба (кламера)</td>
                    <td align="center">1000</td>
                </tr>
                <tr>
                    <td>Перебазировка протеза</td>
                    <td align="center">3050</td>
                </tr>
                <tr>
                    <td>Индивидуальная ложка</td>
                    <td align="center">1100</td>
                </tr>
                <tr>
                    <td>Коррекция протеза </td>
                    <td align="center">500</td>
                </tr>
                <tr>
                    <td colspan="2" align="center"><strong>ДОПОЛНИТЕЛЬНЫЕ РАБОТЫ</strong></td>
                </tr>
                <tr>
                    <td>Коагуляция десневого сосочка</td>
                    <td align="center">100</td>
                </tr>
                <tr>
                    <td>Ретракция десневого края</td>
                    <td align="center">100</td>
                </tr>
                <tr>
                    <td>Изготовление силиконового ключа</td>
                    <td align="center">300</td>
                </tr>
                <tr>
                    <td>Изоляция поля Оплдамм</td>
                    <td align="center">300</td>
                </tr>
                <tr>
                    <td>Фиксация коронки (цемент) 1 ед.</td>
                    <td align="center">250</td>
                </tr>
                <tr>
                    <td>Фиксация коронки (иономерный цемент) 1 ед.</td>
                    <td align="center">350</td>
                </tr>
                <tr>
                    <td>Фиксация изделий из фарфора 1 ед.</td>
                    <td align="center">500</td>
                </tr>
                <tr>
                    <td>Снятие коронки из металла 1 ед.</td>
                    <td align="center">250</td>
                </tr>
                <tr>
                    <td>Снятие коронки из металлокерамики 1 ед. </td>
                    <td align="center">500</td>
                </tr>
                <tr>
                    <td>Шлифовка пломбы, сколов</td>
                    <td align="center">200</td>
                </tr>
                <tr>
                    <td>Починка протезов</td>
                    <td align="center">1500</td>
                </tr>
                <tr>
                    <td colspan="2" align="center"><strong>ЛЕЧЕНИЕ НА ЭЛАЙНЕРАХ</strong></td>
                </tr>
                <tr>
                    <td>До 10 кап (элайнеров) </td>
                    <td align="center">100 000</td>
                </tr>
                <tr>
                    <td>От 11 до 20 кап (элайнеров) </td>
                    <td align="center">150 000</td>
                </tr>
                <tr>
                    <td>От 21 капы (элайнера) </td>
                    <td align="center">180 000</td>
                </tr>
                <tr>
                    <td colspan="2" align="center"> </td>
                </tr>
                <tr>
                    <td>Доставка в лабораторию слепков в пределах области</td>
                    <td align="center">бесплатно</td>
                </tr>
                <tr>
                    <td>Доставка в лабораторию слепков за пределы области</td>
                    <td align="center">400</td>
                </tr>
            </table>
        </div>
    </div>
    <div class="panel panel-default">
        <a class="panel-heading btn" data-toggle="collapse" href="#price_implant" >Имплантология</a>
        <div id="price_implant" class="panel-collapse collapse">
            <table width="100%" border="0" cellspacing="2" cellpadding="0">
                <tr>
                    <th>Услуга</th>
                    <th width="100" align="center">Цена руб.</th>
                </tr>

                <tr>
                    <td>Имплант Alpha Bio, в том числе снятие слепков, без коронки</td>
                    <td align="center">23 700</td>
                </tr>
                <tr>
                    <td>Формирователь десны</td>
                    <td align="center">3 000</td>
                </tr>
                <tr>
                    <td>Полная стоимость Имплант Alpha Bio весь комплект/стандарт без скидки — 44 000 руб.</td>
                    <td align="center"></td>
                </tr>

            </table>
        </div>
    </div>
    <div class="panel panel-default">
        <a class="panel-heading btn" data-toggle="collapse" href="#price_other" >Снимки и прочее</a>
        <div id="price_other" class="panel-collapse collapse">
            <table width="100%" border="0" cellspacing="2" cellpadding="0">
                <tr>
                    <th>Услуга</th>
                    <th width="100" align="center">Цена руб.</th>
                </tr>

                <tr>
                    <td>Радиовизиографическое исследование первичное</td>
                    <td align="center">150</td>
                </tr>
                <tr>
                    <td>Радиовизиографическое исследование повторное</td>
                    <td align="center">80</td>
                </tr>
                <tr>
                    <td>Анестезия </td>
                    <td align="center">250</td>
                </tr>
                <tr>
                    <td>Дополнительная анестезия</td>
                    <td align="center">100</td>
                </tr>
                <tr>
                    <td>Анестезия аппликационная</td>
                    <td align="center">50</td>
                </tr>
            </table>
        </div>
    </div>
    <div class="panel panel-default">
        <a class="panel-heading btn" data-toggle="collapse" href="#price_prevention" >Профилактика</a>
        <div id="price_prevention" class="panel-collapse collapse">
            <table width="100%" border="0" cellspacing="2" cellpadding="0">
                <tr>
                    <th>Услуга</th>
                    <th width="100" align="center">Цена руб.</th>
                </tr>
                <tr>
                    <td>Осмотр, консультация, заполнение мед. документации</td>
                    <td width="100" align="center">250</td>
                </tr>
                <tr>
                    <td>Повторный осмотр</td>
                    <td width="100" align="center">100</td>
                </tr>
                <tr>
                    <td>Урок гигиены по уходу за полостью рта без подбора специальных средств</td>
                    <td width="100" align="center">150</td>
                </tr>
                <tr>
                    <td>Наложение ретрактора (защита десны)</td>
                    <td width="100" align="center">300</td>
                </tr>
                <tr>
                    <td colspan="2" align="center"><strong>РЕМИНЕРАЛИЗУЮЩАЯ ТЕРАПИЯ</strong></td>
                </tr>
                <tr>
                    <td>Повязка при повышенной чувтвительности зубов (Fluokal, глуфторед) 1 зуб</td>
                    <td align="center">150</td>
                </tr>
                <tr>
                    <td>Покрытие лаком Gluma Desensitiser</td>
                    <td align="center">150</td>
                </tr>
                <tr>
                    <td>Снятие слепка 1 челюсть</td>
                    <td align="center">400</td>
                </tr>
                <tr>
                    <td>Обработка гелем ROCS MINERALS </td>
                    <td align="center">50</td>
                </tr>
                <tr>
                    <td colspan="2" align="center"><strong>ПАРОДОНТОЛОГИЯ</strong></td>
                </tr>
                <tr>
                    <td>Антисептическая обработка слизистой оболочки </td>
                    <td align="center">70</td>
                </tr>
                <tr>
                    <td>Наложение лечебного геля  на десну (Метрогил, Холисал) 1 сегмент</td>
                    <td align="center">150</td>
                </tr>
                <tr>
                    <td>Наложение кератопластической пасты на десну Солкосерил 1 сегмент</td>
                    <td align="center">200</td>
                </tr>
                <tr>
                    <td>Наложение лечебной пролонгированной  повязки на десну Диплен - дента 1 сегмент</td>
                    <td align="center">200</td>
                </tr>
                <tr>
                    <td>Шинирование подвижных зубов 2 сегмента (6 зубов)</td>
                    <td align="center">3000</td>
                </tr>
            </table>
        </div>
    </div>
</div>

<p class="text-center mb100">
    Размещённый прайс не является офертой. <br>
    Уточняйте информацию по телефонам: (4932) 903-800, 902-211.
</p>
